...
const campaignCreate = asyncHandler(async (req, res, next) => {
  try {
    const {
      title,
      slug,
    ...
    if (!simpleUrlRegex.test(bannerImage)) {
      res.status(400);
    ...
    if (link) {
      const simpleLinkRegex = /^https?:\/\/.+/i;
      if (!simpleLinkRegex.test(link)) {
        res.status(400);
...
const campaignUpdate = asyncHandler(async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    // Validations for bannerImage with simpler regex
    if (bannerImage) {
      const simpleUrlRegex = /^https?:\/\/.+/i;
      if (!simpleUrlRegex.test(bannerImage)) {
        res.status(400);
    ...
    // Validations for link with simpler regex
    if (link) {
      const simpleLinkRegex = /^https?:\/\/.+/i;
      if (!simpleLinkRegex.test(link)) {
        res.status(400);
